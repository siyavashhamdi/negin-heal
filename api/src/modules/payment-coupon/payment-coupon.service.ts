...
import {
  PaymentCouponCreateGqlInput,
  PaymentCouponDeleteGqlInput,
  PaymentCouponListGqlInput,
  PaymentCouponListSortOptionInput,
  PaymentCouponUpdateGqlInput,
  PaymentCouponValidateGqlInput,
} from "./graphql/inputs";
import {
  PaymentCouponListGqlResponse,
  PaymentCouponListPaginatedOffsetGqlResponse,
  PaymentCouponValidateGqlResponse,
...
type PaymentCouponListRecord = PaymentCoupon & { _id: Types.ObjectId };
...
type PaymentCouponUpdateOperation = {
...
export class PaymentCouponService {
  ...
  async list(
    input: PaymentCouponListGqlInput,
    ...
    const sortOptions = this.resolvePaymentCouponListSortOptions(options?.sort);

    const [coupons, total] = await Promise.all([
      this.paymentCouponModel
        .find(filterQuery)
        .sort(sortOptions)
        .skip(skip)
        .limit(limit)
        .lean<PaymentCouponListRecord[]>()
        .exec(),
      this.paymentCouponModel.countDocuments(filterQuery).exec(),
    ]);
    ...
    return {
      items: coupons.map((coupon) =>
        this.toPaymentCouponListResponse(coupon, usageCountsByCouponId),
      ),
      pagination: {
        limit,
        skip,
  ...
  async create(
    input: PaymentCouponCreateGqlInput,
  ): Promise<PaymentCouponListGqlResponse> {
    const createData = await this.buildCreateData(input);
    const existingCoupon = await this.paymentCouponModel
      .findOne({ code: createData.code })
      .lean()
      .exec();

    if (existingCoupon) {
      throw new ConflictException("Payment coupon code already exists");
    }

    const coupon = await this.paymentCouponModel.create(createData);

    return this.toPaymentCouponListResponse(
      coupon.toObject() as PaymentCouponListRecord,
      new Map(),
    );
  }

  async update(
    input: PaymentCouponUpdateGqlInput,
  ): Promise<PaymentCouponListGqlResponse> {
    const existingCoupon = await this.paymentCouponModel
      .findOne({
        _id: input.id,
    ...
    if (!existingCoupon) {
      throw new NotFoundException("Payment coupon not found");
    }

    const updateOperation = await this.buildUpdateOperation(
      input,
      existingCoupon.toObject() as PaymentCouponListRecord,
    );
    ...
    const updatedCoupon = await this.paymentCouponModel
    ...
    return this.toPaymentCouponListResponse(
      updatedCoupon.toObject() as PaymentCouponListRecord,
      usageCountsByCouponId,
    );
  }

  async delete(input: PaymentCouponDeleteGqlInput): Promise<void> {
    const deletedCoupon = await this.paymentCouponModel
  ...
  private async buildCreateData(
    input: PaymentCouponCreateGqlInput,
  ): Promise<PaymentCouponCreateData> {
  ...
  private validateCouponDiscount(
    ...
    if (
      discountType === PaymentCouponDiscountType.PERCENTAGE &&
      discountValue > 100
    ) {
      throw new BadRequestException(
  ...
  private async buildUpdateOperation(
    input: PaymentCouponUpdateGqlInput,
    existingCoupon: PaymentCouponListRecord,
  ): Promise<PaymentCouponUpdateOperation> {
  ...
  private buildListFilterQuery(
    filters?: PaymentCouponListGqlInput["filters"],
  ): FilterQuery<PaymentCoupon> {
    const query: FilterQuery<PaymentCoupon> = {
      $and: [
        {
          $or: [
            { "audit.deletedAt": null },
  ...
  private resolvePaymentCouponListSortOptions(
    sort?: PaymentCouponListSortOptionInput,
  ): Record<string, 1 | -1> {
    const sortOptions = buildSortOptions<PaymentCouponListSortField>(
      sort ?? {},
      {
  ...
  private toPaymentCouponListResponse(
    coupon: PaymentCouponListRecord,
  ...
  private addListContainsFilter(
    query: FilterQuery<PaymentCoupon>,
    path: string,
    value?: string,
  ): void {
    if (value?.trim()) {
  ...
  private addListOrFilter(
    query: FilterQuery<PaymentCoupon>,
    conditions: FilterQuery<PaymentCoupon>[],
  ): void {
    query.$and = [
