...
export function sanitizeLatinAuthIdentity(value: string): string {
  return toWesternDigits(value)
    .replace(NON_AUTH_IDENTITY_START, "")
    .replace(NON_LATIN_AUTH_IDENTITY, "");
}
...
export function isLatinEmailValue(value: string): boolean {
  const trimmed = value.trim();
  return (
    trimmed.length > 0 &&
 ...
 */
export function tryNormalizeAuthIdentityMobile(
  value: string,
): string | undefined {
...
export function isValidAuthIdentityMobileInput(value: string): boolean {
  const trimmed = value.trim();
...
export function normalizeAuthIdentityMobileForSubmit(
  value: string,
): string | undefined {
  const normalized = normalizeIdentityValue(value);
...
export function isAuthIdentityMobileMode(value: string): boolean {
  const normalized = normalizeIdentityValue(value);
  ...
  // 0 / 09… → mobile unless letters are present (then username).
  if (normalized === "0" || normalized.startsWith("09")) {
    return !HAS_ALPHA_LETTER.test(normalized);
  }

  // Any + prefix (without @) → mobile mode.
  if (normalized.startsWith("+")) {
    return true;
  }

  // 9… (not 09) → mobile only for exact 9xxxxxxxxx or 989xxxxxxxxx.
...
export function isAuthIdentityMobile(value: string): boolean {
  return tryNormalizeAuthIdentityMobile(value) !== undefined;
}

export function detectAuthIdentityKind(identity: string): AuthIdentityKind {
...
export function isValidAuthIdentity(value: string): boolean {
  const trimmed = value.trim();
...
export function resolveInvalidAuthIdentityErrorKind(
  value: string,
...
export function normalizeAuthIdentityForSubmit(value: string): string {
  const sanitized = sanitizeLatinAuthIdentity(value.trim());
