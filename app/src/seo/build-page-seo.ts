...
export function buildPageSeo(input: BuildPageSeoInput): ResolvedPageSeo {
  ...
  const titleKey = underConstruction ? "pages.underConstruction.pageTitle" : resolvePageTitleKey(pathname);
  ...
  const interpolation =
    override?.title?.trim() ? { title: override.title.trim() } : undefined;

  const description = buildSeoDescription(
    override?.description?.trim() ||
      readPageSeoField(t, pageKey, "description", interpolation),
  );
  ...
  const imageUrl = resolveAbsoluteUrl(
    appUrl,
    override?.image?.trim() || DEFAULT_OG_IMAGE_PATH,
  );
  ...
  const twitterCard = (override?.twitterCard ??
    t("seo.defaults.twitterCard")) as TwitterCardType;
